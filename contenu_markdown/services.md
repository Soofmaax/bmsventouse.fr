# Services

## Meta informations
- URL : https://bmsventouse.fr/services/
- Title tag : Nos Services de Logistique Audiovisuelle | BMS Ventouse
- Meta description : Découvrez nos services experts pour tournages et événements : ventousage, sécurité plateaux, régie technique, convoyage national. Solutions sur mesure pour une production sereine.

## Contenu principal

### Héros
- Titre : Solutions Logistiques Complètes pour Productions Audiovisuelles
- Texte : Que vous soyez régisseur de long métrage, de série, de publicité ou d'événementiel, nous avons la solution. Basés en Seine‑Saint‑Denis (93), nous intervenons principalement à Paris et en Île‑de‑France avec des tarifs directs, sans surcoût d’agence.

### Catalogue de prestations (cartes)
- Sécurité Plateaux & Gardiennage  
  - Agents spécialisés, contrôle d’accès, gestion des flux, gardiennage de matériel et véhicules.  
  - Options SSIAP et maîtres‑chiens.  
  - Liens : /securite-plateaux/, /securite-gardiennage/, /realisations/
- Loges & Confort  
  - Cars loges et loges mobiles, espaces de maquillage, repos, sanitaires intégrés.  
  - Lien : /loges-confort/
- Cantine & Catering  
  - Service de cantine et traiteurs partenaires, menus adaptés (végétarien, végan, etc.).  
  - Liens : /cantine-catering/, /services/
- Ventousage & Stationnement  
  - Gestion d’AOT, neutralisation de stationnement, pose de panneaux, affichage riverains, signalisation temporaire.  
  - Liens : /ventousage/, /ventousage-paris/, /affichage-riverains/, /signalisation-barrierage/, /prix-ventousage-paris/
- Convoyage Véhicules & Décors  
  - Convoyage de véhicules techniques (jusqu’à 22 m³, poids lourds) et transport de décors/matériels entre sites de tournage.  
  - Liens : /convoyage-vehicules-decors/, /transport-materiel-audiovisuel-paris/, /contact/
- Logistique Événementielle (mode, publicité, sport)  
  - Logistique de défilés, shootings, événements institutionnels, parades, etc.  
  - Exemple : Flamme Olympique, parades culturelles.  
  - Lien : /realisations/
- Régie & Matériel  
  - Agents de régie, manutentionnaires, installation de loges/cantines, mise en relation pour location de matériel et consommables.  
  - Lien : /regie-materiel/
- Solutions Packagées & Offres Partenaires  
  - Packages logistique + sécurité + régie, ajustés au besoin réel.  
  - Tarifs directs sans surcoût d’agence, devis poste par poste.  
  - Lien : /contact/

### Avantages inclus
- Devis gratuit sous 24h ouvrées sur les demandes standard.
- Signalisation (pose de panneaux, affichage, périmètre) souvent offerte après acceptation du devis (cas précisés par échange).
- Support 7j/7 pour ajuster le dispositif selon la réalité plateau.
- Tarifs directs, sans surcoût d’agence.

### CTA
- Devis gratuit (Contact) → /contact/
- WhatsApp Direct → https://wa.me/33646005642

## Éléments notables
- Call-to-actions : Devis, WhatsApp, liens vers pages services détaillées.
- Formulaires : la prise de contact se fait via la page /contact/.
- Images importantes :
  - service-securite-custom.jpg
  - service-ventousage-custom.jpg
  - service-convoyage-custom.jpg
  - service-loges-custom.jpg
  - service-cantine.jpg
  - service-evenementiel.jpg
  - service-package.jpg
- Liens internes : Réalisations, Ventousage Paris, Affichage riverains, Signalisation, Gardiennage, Convoyage, Régie, Cantine.

## Problèmes détectés
- Aucun bloquant ; la page services actuelle est cohérente avec ces contenus.